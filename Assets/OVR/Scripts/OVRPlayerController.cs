...
public class OVRPlayerController : OVRComponent
{
	...
	public Quaternion Direction    = new UnityEngine.Quaternion();
	...
	new public virtual void Update()
	{
		...
		OVRDevice.GetOrientation (0 ,ref Direction);
	...
	// NAH OK WE MOVIN IN THE AIR NOW SCREW YOU PHYSIX
	...
	//		MoveScale = 0.0f;
		...
		// charge as if rockets were taped onto our nose
		if(Direction != null){
			if (moveCharge){
				MoveThrottle += DirXform.TransformDirection(Vector3.forward * Rocket * Mathf.Cos(Direction.eulerAngles.x % 90));
				MoveThrottle += DirXform.TransformDirection(Vector3.up * Rocket * Mathf.Sin(Direction.eulerAngles.x % 90));
			}
		}
